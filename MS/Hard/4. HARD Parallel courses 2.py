...
class Solution:
    ...
    def recurse(self, mask, in_degrees):
        ...
        if (mask, in_degrees) in self.cache:
            return self.cache[(mask, in_degrees)]
        
        # all the nodes that *can* be taken now, following both the properties
        # mask is all nodes set (1) yet to be taken, mask & 1 << i says if ith node is 
        # yet to be taken
        nodes = [i for i in range(self.n) if mask & 1 << i and in_degrees[i] == 0]
        
        ans = float('inf')
        self.cache[(mask, in_degrees)] = float('inf')
        ...
        self.cache[(mask, in_degrees)] = ans
        return ans
